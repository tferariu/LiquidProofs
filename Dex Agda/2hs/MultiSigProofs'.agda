...
open import MultiSig hiding (insert)
...
open import ListInsertLib (PubKeyHash) _==_ ==ito≡ =/=ito≢ 
...
rewriteSigs : ∀ {sigs sigs'} → sigs ≡ sigs' → Unique sigs → Unique sigs'
rewriteSigs refl p2 = p2
...
prop2 {d = d} {sigs = sigs}
  ...
  = lemmaMultiStep par s1 s' s2 (makeIs (authSigs par)) [ Pay ]   
    ...
    (cons (TPay refl (≤-trans p5 (uniqueInsertLemma2 (authSigs par) sigs p4 (rewriteSigs (sameSigs (sym p1)) p6))) refl refl refl refl) root)
      where
...
sameInsert : ∀ sig sigs → MultiSig.insert sig sigs ≡ insert sig sigs
sameInsert sig [] = refl
sameInsert sig (x ∷ sigs) with sig == x in eq
...| true = refl
...| false = cong (x ∷_) (sameInsert sig sigs)
...
validatorImpliesTransition par (Collecting v pkh d sigs) (Add sig)
  record { inputVal = inputVal ;
           ...
           payAmt = payAmt ;
           signature = signature } pf 
  rewrite sameInsert sig sigs
  | p1 (sig == signature) (query sig (authSigs par)) (v ≡ᵇ v') (pkh == pkh') (d ≡ᵇ d') (sigs' == (insert sig sigs)) outputVal inputVal pf
...
transitionImpliesValidator par (Collecting v pkh d sigs) (Add sig)
  ...
  rewrite v=v inputVal | i=i sig | ∈toQuery p1 | v=v v | i=i pkh
        | v=v d | sameInsert sig sigs | l=l (insert sig sigs) = refl
