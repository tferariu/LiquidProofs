...
totalF (Running par adr oref d i ctx v r amt pkh x) = agdaValidator par d i ctx
...
totalR (Running par adr oref d i ctx v r amt pkh x) = getPar par adr oref ⊢ getS d ctx ~[ i ]~> getS' ctx
totalR (Final par adr oref d i ctx x) = getPar par adr oref ⊢ getS d ctx ~[ i ]~| getS' ctx

totalEquiv : totalF ≈ totalR
totalEquiv = record { to = λ { {Initial par adr oref d i ctx p} x → mintingImpliesStart adr oref i ctx p x ;
                               {Running par adr oref d i ctx v r amt pkh p} x → validatorImpliesTransition par d i ctx p x ;
                               {Final par adr oref d i ctx p} x → bothImplyClose par d adr oref i ctx x } ;
                    from = λ { {Initial par adr oref d i ctx p} x → startImpliesMinting adr oref i ctx x ;
                               {Running par adr oref d i ctx v r amt pkh p} x → transitionImpliesValidator par d i ctx x ;
                               {Final par adr oref d i ctx p} x → closeImpliesBoth par d adr oref i ctx x } }
