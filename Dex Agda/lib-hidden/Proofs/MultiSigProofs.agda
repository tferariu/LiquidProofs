...
validatorImpliesTransition par (tok , Collecting v pkh d sigs) Pay ctx@record { inputVal = inputVal ; outputVal = outputVal ; outputDatum = (tok' , Holding) ; time = time ; signature = signature ; continues = continues ; inputRef = inputRef ; tokenIn = true ; tokenOut = true ; mint = mint ; tokAssetClass = tokAssetClass } nc pf
  ...
  sym (==ito≡ v (payVal ctx) (go (pkh == (payAdr ctx)) (get (go continues (go ((lengthNat sigs) >= (nr par)) pf))))) |
  ...
  (sym (==ito≡ v (payVal ctx) (go (pkh == (payAdr ctx)) (get (go continues (go ((lengthNat sigs) >= (nr par)) pf)))))) refl refl
...
transitionImpliesValidator par (tok , Collecting v pkh d sigs) Pay record { inputVal = .(outputVal + v) ; outputVal = outputVal ; outputDatum = .(tok , Holding) ;  continues = .true ; tokenIn = .true ; tokenOut = .true } (TPay refl p2 refl refl refl refl p7 refl refl refl)
