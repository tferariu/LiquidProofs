...
validStateInitial : ∀ {s par}
  -> par ⊢ s
  -> Valid s
validStateInitial {record { datum = .(_ , []) ; value = .0 ; tsig = tsig₁ ; continues = continues₁ ; spends = spends₁ ; hasToken = hasToken₁ ; mint = mint₁ ; token = token₁ }} (TStart refl refl p3 p4 p5 p6 p7) = allNil , p4 , p7

validStateTransition : ∀ {s s' : State} {i}
  -> Valid s
  -> s ~[ i ]~> s'
  -> Valid s'
validStateTransition {record { datum = tok , map ; value = value₁ ; tsig = tsig₁ ; continues = continues₁ ; spends = spends₁ ; hasToken = hasToken₁ ; mint = mint₁ ; token = token₁ }} {record { datum = .(_ , insert _ 0 map) ; value = value₂ ; tsig = tsig₂ ; continues = continues₂ ; spends = spends₂ ; hasToken = hasToken₂ ; mint = mint₂ ; token = token₂ }} (fst , snd , thd) (TOpen refl p2 p3 refl p5 p6 p7 p8 p9) = (lem map 0 refl fst) , p7 , p9
...
validStateTransition {record { datum = tok , map ; value = .(value₁ + val) ; tsig = tsig₁ ; continues = continues₁ ; spends = spends₁ ; hasToken = hasToken₁ ; mint = mint₁ ; token = token₁ }} {record { datum = .(_ , insert _ (v - val) map) ; value = value₁ ; tsig = tsig₂ ; continues = continues₂ ; spends = spends₂ ; hasToken = hasToken₂ ; mint = mint₂ ; token = token₂ }} (fst , snd , thd) (TWithdraw {val = val} {v = v} refl p2 p3 p4 p5 refl refl p8 p9 p10 p11) = (lem map (v - val) (diffLemma v val p5 p4) fst) , p9 , p11
validStateTransition {record { datum = tok , map ; value = value₁ ; tsig = tsig₁ ; continues = continues₁ ; spends = spends₁ ; hasToken = hasToken₁ ; mint = mint₁ ; token = token₁ }} {record { datum = .(_ , insert _ (v + val) map) ; value = value₂ ; tsig = tsig₂ ; continues = continues₂ ; spends = spends₂ ; hasToken = hasToken₂ ; mint = mint₂ ; token = token₂ }} (fst , snd , thd) (TDeposit {val = val} {v = v} refl p2 p3 p4 refl p6 p7 p8 p9 p10) = lem map (v + val) (sumLemma v val p4 (geqLem map v fst p3)) fst , p8 , p10
validStateTransition {record { datum = tok , map ; value = value₁ ; tsig = tsig₁ ; continues = continues₁ ; spends = spends₁ ; hasToken = hasToken₁ ; mint = mint₁ ; token = token₁ }} {record { datum = .(_ , insert from (vF - val) (insert to (vT + val) map)) ; value = value₂ ; tsig = tsig₂ ; continues = continues₂ ; spends = spends₂ ; hasToken = hasToken₂ ; mint = mint₂ ; token = token₂ }} (fst , snd , thd) (TTransfer {from} {to} {val} {vF = vF} {vT = vT} refl p2 p3 p4 p5 p6 p7 refl p9 p10 p11 p12 p13) = lem (insert to (vT + val) map) (vF - val) (diffLemma vF val p6 p5) (lem map (vT + val) (sumLemma vT val p5 (geqLem map vT fst p4)) fst) , p11 , p13
...
getGeq : ∀ {s x tok map v sig spn mnt tok'}
         ...
         -> Valid s
         -> snd x ≥ emptyValue
getGeq refl (allCons {{i}} {{is}} , x , y) = geqto≤ i
...
prop1 s@record { datum = (tok , x ∷ map) ; value = value ; tsig = tsig ; continues = continues ; spends = spends ; hasToken = hasToken ; mint = mint ; token = token } s'@record { datum = (tok' , map') ; value = value' ; tsig = tsig' ; continues = continues' ; spends = spends' ; hasToken = hasToken' ; mint = mint' ; token = token' } refl refl refl refl refl refl refl refl refl refl refl refl p
  = cons {s' = st} (TWithdraw refl refl (lookupProp1 (n=n (fst x)) refl) (getGeq {s} refl p) ≤-refl (deleteProp1 (n=n (fst x))  (rewriteLabel (fst x) (snd x) map)) (+-comm (sumVal map) (x .snd)) refl refl refl refl) 
...
validatorImpliesTransition (tok , map) (Withdraw pkh val) ctx iv pf | Just v | tok' , map'
  rewrite sym (==to≡ tok' tok (get (go (geq v val) (go (geq val emptyValue) (get (go (pkh == (sig ctx))
             ...
             (go (continuing ctx) (go (hasTokenOut ctx) (go (hasTokenIn ctx) pf))))))))) | sym eq2
