...
validatorImpliesTransition par d (Update v r) ctx p1 p2
  = TUpdate (==to≡ (owner (snd d)) (signature ctx) (get (go (checkTokenIn (d .fst) ctx) p2)))
...
validatorImpliesTransition par d (Exchange amt pkh) ctx p1 p2
  ...
  (==to≡ (payTo ctx) (owner (snd d)) (get (get (go (newDatum ctx == ((d .fst) , (d .snd)))
  ...
  (go (oldValue ctx == newValue ctx <> (assetClassValue (sellC par) amt)) (go (checkTokenIn (d .fst) ctx) p2))))))
  (==to≡ (buyTo ctx) pkh (get (get (go (checkPayment par amt (d .snd) ctx) (go (newDatum ctx == ((d .fst) , (d .snd)))
  (go (oldValue ctx == newValue ctx <> (assetClassValue (sellC par) amt)) (go (checkTokenIn (d .fst) ctx) p2)))))))
  (==ito≡ (assetClassValueOf (buyVal ctx) (sellC par)) amt (get (go (buyTo ctx == pkh)
...
bothImplyClose par d adr oref top ctx p | False | True
  = TClose (==to≡ (owner (snd d)) (signature ctx) (go (not (checkTokenOut (newDatum ctx .fst) ctx)) (go (not (continuing ctx))
    (go (checkTokenIn (d .fst) ctx) (get p))))) (==ito≡ (mint ctx) (negsuc 0) eq') refl
    (unNot (continues ctx) ((get (go ((checkTokenIn (d .fst) ctx)) (get p))))) (get (get p))
    (unNot ((eqInteger (assetClassValueOf (outputVal ctx) (outputDatum ctx .fst)) (+ 1)))
...
removeClose record { par = par ; adr = adr ; oref = oref ; dat = dat ; inp = Close ; ctx = ctx } p1 p2 = ⊥-elim (p1 (==ito≡ (mint ctx) (negsuc 0) (get (go (not (continues ctx)) (go (checkTokenIn (fst dat) ctx) p2)))))
