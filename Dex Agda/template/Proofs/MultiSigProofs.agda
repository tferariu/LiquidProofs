...
liquidity par
  s@record { datum = (tok , Holding) ; value = value ; outVal = outVal ; now = now ; tsig = tsig ; continues = continues ; spends = spends ; hasToken = hasToken ; mint = mint ; token = token } pkh (Hol refl p p') (p2 , p3) with geq value minValue in eq
...| false = ⟨ s' , ⟨ [ Close ] , ((fin (TClose refl (ltToGt (value) (minValue) (notGeqToLt (value) (minValue) eq)) p refl p' refl refl) root) , refl) ⟩ ⟩ 
     where
       s' : State
       s' = record
             { datum = tok , Holding
...
...| true  = ⟨ s'' , ⟨ ((Propose value pkh 0) ∷ ((makeIs (authSigs par)) ++ [ Pay ])) ,
             (cons (TPropose (geq-refl value) eq refl refl refl N.z≤n p refl p' refl) 
             (prop2 s' s'' par (Col refl (geq-refl value) eq root refl refl) refl refl refl (sym (addValIdL value)) (p2 , p3) refl refl refl refl refl) , refl) ⟩ ⟩  --(sym (addValIdL value))
     where
       s'' = record
              { datum = tok , Holding
              ; value = emptyValue
...
liquidity par s@record { datum = (tok , Collecting v' pkh' d' sigs') ; value = value ; outVal = outVal ; now = now ; tsig = tsig ; continues = continues ; spends = spends ; hasToken = hasToken ; mint = mint ; token = token } pkh (Col refl p2 p3 p4 p5 p6) (p7 , p8) with geq value minValue in eq
...| false rewrite geqTrans value v' minValue p2 p3  = ⊥-elim (get⊥ eq) 
...
getS' (tok , Collecting v pkh d sigs) ctx = record
             { datum = newDatum ctx
             ; value = newValue ctx
             ; outVal = getPayment pkh (getPayments ctx)
             ; now = now' ctx
             ; tsig = sig ctx
             ; continues = continuing ctx
             ; spends = iRef ctx
...
validatorImpliesTransition par (tok , Holding) (Propose v pkh d) record { inputVal = inputVal ; outputVal = outputVal ; outputDatum = (tok' , Holding) ; time = time ; signature = signature ; continues = continues ; inputRef = inputRef ; tokenIn = true ; tokenOut = true ; mint = mint ; tokAssetClass = tokAssetClass } nc pf
  = ⊥-elim (&&5false (eqValue outputVal inputVal) (geq inputVal v) (geq v minValue) (ltNat d (addNat time (maxWait par)) || eqNat (addNat time (maxWait par)) d) continues  pf)
validatorImpliesTransition par (tok , Holding) (Propose v pkh d) ctx@record { inputVal = inputVal ; outputVal = outputVal ; outputDatum = (tok' , Collecting v' pkh' d' sigs') ; time = time ; signature = signature ; continues = continues ; inputRef = inputRef ; tokenIn = true ; tokenOut = true ; mint = mint ; tokAssetClass = tokAssetClass } nc pf
  rewrite sym (==vto≡ v v' (get (go continues (go (notTooLate par d ctx) (go (geq v minValue) (go (geq inputVal v) (go (eqValue outputVal inputVal) pf))))))) | --(ltInteger v inputVal || eqInteger inputVal v)
  sym (==to≡ pkh pkh' (get (go (eqValue v v') (go continues (go (notTooLate par d ctx) (go (geq v minValue) (go (geq inputVal v) (go (eqValue outputVal inputVal) pf)))))))) |
  sym (==to≡ d d' (get (go (pkh == pkh') (go (eqValue v v') (go continues (go (notTooLate par d ctx) (go (geq v minValue) (go (geq inputVal v) (go (eqValue outputVal inputVal) pf))))))))) |
  (==lto≡ sigs' [] (get (go (d == d') (go (pkh == pkh') (go (eqValue v v') (go continues (go (notTooLate par d ctx) (go (geq v minValue) (go (geq inputVal v) (go (eqValue outputVal inputVal) pf)))))))))) |
  (==to≡ tok tok' (go (sigs' == []) (go (d == d') (go (pkh == pkh') (go (eqValue v v') (go continues (go (notTooLate par d ctx) (go (geq v minValue) (go (geq inputVal v) (go (eqValue outputVal inputVal) pf))))))))))
  = TPropose (get (go (eqValue outputVal inputVal) pf))
     ...
     (≤ᵇNto≤N' (get (go (geq v minValue) (go (geq inputVal v) (go (eqValue outputVal inputVal) pf))))) refl 
     (get (go (notTooLate par d ctx) (go (geq v minValue) (go (geq inputVal v) (go (eqValue outputVal inputVal) pf))))) refl refl 
...
validatorImpliesTransition par (tok , Collecting v pkh d sigs) Pay ctx@record { inputVal = inputVal ; outputVal = outputVal ; outputDatum = (tok' , Holding) ; time = time ; signature = signature ; continues = continues ; inputRef = inputRef ; tokenIn = true ; tokenOut = true ; mint = mint ; tokAssetClass = tokAssetClass } nc pf
  rewrite ==to≡ pkh (payAdr ctx) (get (get (go continues (go ((lengthNat sigs) >= (nr par)) pf)))) |
  ==to≡ tok tok' (go (inputVal == (addValue outputVal v)) (go (checkPayment pkh v ctx) (go continues (go ((lengthNat sigs) >= (nr par)) pf))))
  ...
  (sym (==vto≡ v (payVal ctx) (go (pkh == (payAdr ctx)) (get (go continues (go ((lengthNat sigs) >= (nr par)) pf)))))) refl refl 
...
validatorImpliesTransition par (tok , Collecting v pkh d sigs) (Add sig) record { inputVal = inputVal ; outputVal = outputVal ; outputDatum = (tok' , Collecting v' pkh' d' sigs') ; time = time ; signature = signature ; continues = continues ; inputRef = inputRef ; tokenIn = true ; tokenOut = true ; mint = mint ; tokAssetClass = tokAssetClass } nc pf
  ...
  (==to≡ tok tok' (go (sigs' == (insert sig sigs)) (go (d == d') (go (pkh == pkh') (go (eqValue v v') (go continues (go (query sig (authSigs par)) (go (sig == signature) (go (eqValue outputVal inputVal) pf)))))))))
...
bothImplyClose par d@(tok , Holding) adr oref top ctx@record { tokenOut = false ; mint = .-1 } refl p = TClose refl (get (go (hasTokenIn ctx) (get p))) refl (unNot (continuing ctx) (go (agdaValidator par d Close ctx) p)) (get (get p)) refl refl
...
transitionImpliesValidator par (tok , .Holding) (Propose v pkh d) record { inputVal = inputVal ; outputVal = .(inputVal) ; outputDatum = .(tok , Collecting v pkh d []) ; continues = .true ; inputRef = inputRef ; tokenIn = .true ; tokenOut = .true } (TPropose p1 p2 refl refl refl p6 p7 refl refl refl)
  rewrite v=v inputVal | v=v v | n=n pkh | n=n d | n=n tok | p1 | p2 | ≤Nto≤ᵇN p6 = refl 
transitionImpliesValidator par (tok , Collecting v pkh d sigs) (Add sig) record { inputVal = inputVal ; outputVal = .(inputVal) ; outputDatum = .(tok , Collecting v pkh d (insert sig sigs)) ; signature = .sig ; continues = .true ; tokenIn = .true ; tokenOut = .true } (TAdd p1 refl refl refl refl p6 refl refl refl)
  rewrite v=v inputVal | v=v v | n=n pkh | n=n d | n=n tok | l=l (insert sig sigs) | n=n sig | ∈toQuery p1 = refl
transitionImpliesValidator par (tok , Collecting v pkh d sigs) Pay record { inputVal = .(addValue outputVal v) ; outputVal = outputVal ; outputDatum = .(tok , Holding) ;  continues = .true ; tokenIn = .true ; tokenOut = .true } (TPay refl p2 refl refl refl refl p7 refl refl refl)
  rewrite n=n pkh | v=v v | v=v (addValue outputVal v) | n=n tok | lengthToLengthNat (nr par) sigs p2 = refl
transitionImpliesValidator par (tok , Collecting v pkh d sigs) Cancel record { inputVal = inputVal ; outputVal = .(inputVal) ; outputDatum = .(tok , Holding) ; continues = .true ; inputRef = inputRef ; tokenIn = .true ; tokenOut = .true } (TCancel p1 refl refl refl p5 refl refl refl)
  rewrite v=v inputVal | n=n tok | <Nto<ᵇN p1 = refl
...
closeImpliesBoth : ∀ (par : Params) (d : Label) (adr : Address) (oref : TxOutRef) (top : ⊤) (ctx : ScriptContext)
               -> (getPar par adr oref ⊢ getS d ctx ~[ Close ]~| getS' ctx)
               -> (agdaValidator par d Close ctx && agdaPolicy adr oref top ctx) ≡ true
closeImpliesBoth par (tok , Holding) adr oref top record { continues = .false ; tokenIn = .true ; tokenOut = .false ; mint = .-1 } (TClose refl p2 p3 refl refl refl refl) rewrite p2 = refl 
...
removeClose record { par = par ; adr = adr ; oref = oref ; dat = (fst , Holding) ; inp = Close ; ctx = record { inputVal = inputVal ; continues = continues ; tokenIn = hasTokenIn ; tokenOut = false ; mint = mint ; tokAssetClass = tokAssetClass } } p1 p2 = ⊥-elim (p1 (==ito≡ mint (negsuc 0) (go (not continues) (go (gt minValue inputVal) (go hasTokenIn p2)))))
