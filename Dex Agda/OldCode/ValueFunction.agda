...
minValue : Value
minValue = λ ac → if ac == Ada then 2 else emptyValue ac

--insert : AssetClass' -> Integer -> Value
--insert ac amt = {!!}

addValue : Value -> Value -> Value
addValue v1 v2 = λ ac → v1 ac + v2 ac

subValue : Value -> Value -> Value
subValue v1 v2 = λ ac → v1 ac - v2 ac

negValue : Value -> Value
negValue v = λ z → negateInteger (v z)

compareWith : (Integer -> Integer -> Bool) -> Value -> Value -> Bool
compareWith f v1 v2 = (f (v1 Ada) (v2 Ada)) &&
                      (f (v1 T1)  (v2 T1)) &&
                      (f (v1 T2)  (v2 T2)) &&
                      (f (v1 T3)  (v2 T3)) &&
                      (f (v1 T4)  (v2 T4)) &&
...
eqValue = compareWith _==_

leq : Value -> Value -> Bool
leq = compareWith _<=_

lt : Value -> Value -> Bool
lt v1 v2 = leq v1 v2 && not (eqValue v1 v2)

geq : Value -> Value -> Bool
geq = compareWith _>=_ 

gt : Value -> Value -> Bool
gt v1 v2 = geq v1 v2 && not (eqValue v1 v2)
...
instance
  ...
  iNumberValue = record { Constraint = λ x → ⊤ ; fromNat = λ n → λ x → Integer.pos n }
...
addValIdL a = extensionality (λ x → addIdL (a x))

addValIdR : ∀ (a : Value) -> addValue a emptyValue ≡ a
addValIdR a = extensionality (λ x → addIdR (a x))

v=v : ∀ (v : Value) -> eqValue v v ≡ True
v=v a rewrite i=i (a Ada) | i=i (a T1)
            | i=i (a T2) | i=i (a T3)
            | i=i (a T4) | i=i (a T5) = refl


commAddV : ∀ (a b : Value) -> addValue a b ≡ addValue b a
commAddV a b = extensionality (λ {x → addComm (a x) (b x)}) 
...
data eqValue' : Value → Value → Set where
  Always : ∀ {v1 v2}
         → (∀ (ac : AssetClass') -> ((v1 ac) == (v2 ac)) ≡ True)
         → eqValue' v1 v2


--eq? : ∀ {v1 v2} -> Dec (eqValue' v1 v2)
--eq? {v1} {v2} = {!!} --{!!} because {!!}

data lt' : Value → Value → Set where
  Always : ∀ {v1 v2}
         → (∀ (ac : AssetClass') -> ((v1 ac) < (v2 ac)) ≡ True)
         → lt' v1 v2
